...
public class InstanceUploaderTask extends AsyncTask<Long, Integer, HashMap<String, String>> {
	...
	protected HashMap<String, String> doInBackground(Long... values) {
		...
		Cursor c = MIntel.getInstance().getContentResolver().query(InstanceColumns.CONTENT_URI, null, selection, selectionArgs, null);
		...
		if (c.getCount() > 0) {
			...
			next_submission: while (c.moveToNext()) {
				...
				if (urlString == null) {
					SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(MIntel.getInstance());
