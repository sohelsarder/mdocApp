...
import com.mpower.mintel.android.listeners.AdvanceToNextListener;
import com.mpower.mintel.android.listeners.FormLoaderListener;
import com.mpower.mintel.android.listeners.FormSavedListener;
import com.mpower.mintel.android.logic.FormController;
import com.mpower.mintel.android.logic.PropertyManager;
...
import com.mpower.mintel.android.tasks.FormLoaderTask;
import com.mpower.mintel.android.tasks.SaveToDiskTask;
import com.mpower.mintel.android.utilities.FileUtils;
import com.mpower.mintel.android.views.MIntelView;
import com.mpower.mintel.android.widgets.QuestionWidget;
...
public class FormEntryActivity extends Activity implements AnimationListener,
		...
		if (data instanceof FormLoaderTask) {
			...
			if (intent != null) {
				...
				if (getContentResolver().getType(uri) == InstanceColumns.CONTENT_ITEM_TYPE) {
					Cursor instanceCursor = this.managedQuery(uri, null, null,
							null, null);
					if (instanceCursor.getCount() != 1) {
						...
						if (formCursor.getCount() == 1) {
							...
							this.createErrorDialog(
									"Parent form does not exist", EXIT);
							return;
						} else if (formCursor.getCount() > 1) {
							this.createErrorDialog(
	...
	protected void onActivityResult(int requestCode, int resultCode,
		...
		case IMAGE_CAPTURE:
			...
			// file
			File fi = new File(MIntel.TMPFILE_PATH);
        ...
        case INSTANCE_UPLOADER:
            if (intent.getBooleanExtra(FormEntryActivity.KEY_SUCCESS, false)) {
            	// ##### TODO Delete the data in case of a success
            	
            	finish();
            }else {
            	createErrorDialog("ফর্ম পাঠানো হয়নি। আপনি \"ফর্ম এর ডাটা পাঠান\" এ গিয়ে ফর্ম পাঠিয়ে  দিন।", true);
            	//finish();
            }
            break;
        default:
	...
	private boolean saveAnswersForCurrentScreen(boolean evaluateConstraints) {
		...
		if (mFormController.getEvent() == FormEntryController.EVENT_QUESTION
				|| (mFormController.getEvent() == FormEntryController.EVENT_GROUP && mFormController
						.indexIsInFieldList())) {
			LinkedHashMap<FormIndex, IAnswerData> answers = ((MIntelView) mCurrentView)
					.getAnswers();
	...
	public boolean onContextItemSelected(MenuItem item) {
		 ...
		 * clicked on.
		 */
		for (QuestionWidget qw : ((MIntelView) mCurrentView).getWidgets()) {
	...
	private View createView(int event) {
		...
		case FormEntryController.EVENT_BEGINNING_OF_FORM:
			...
			Cursor c = managedQuery(FormsColumns.CONTENT_URI, projection,
		...
		case FormEntryController.EVENT_GROUP:
			...
			for (QuestionWidget qw : mintelv.getWidgets()) {
						...
						case 1: // discard changes and exit
							...
							if (c.getCount() < 1) {
								...
								if (audioCursor.getCount() > 0) {
									...
									String id = audioCursor.getString(imageCursor
											.getColumnIndex(Images.ImageColumns._ID));
								...
								// video
								Cursor videoCursor = getContentResolver()
										.query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI,
												projection, where, null, null);
								if (videoCursor.getCount() > 0) {
									videoCursor.moveToFirst();
									String id = videoCursor.getString(imageCursor
											.getColumnIndex(Images.ImageColumns._ID));
	...
	public void loadingComplete(FormController fc) {
		...
		if (mInstancePath == null) {
			...
			String path = MIntel.INSTANCES_PATH + "/" + file + "_" + time;
...
//					.getDefaultSharedPreferences(this);
		...
		Cursor c = getContentResolver().query(InstanceColumns.CONTENT_URI,
	...
	private void finishReturnInstance() {
		...
		if (Intent.ACTION_PICK.equals(action)
			...
			String selection = InstanceColumns.INSTANCE_FILE_PATH + "=?";
			String[] selectionArgs = { mInstancePath };
			Cursor c = managedQuery(InstanceColumns.CONTENT_URI, null,
					selection, selectionArgs, null);
			if (c.getCount() > 0) {
				// should only be one...
				c.moveToFirst();
				...
				Log.w("INSTANCE SAVED AND DATA RETRIEVED", "Returned Uri: "+instance.toString());
				long[] instanceIDs = {Long.parseLong(id)};
	...
	private void deleteSelectedFiles(ArrayList<Long> mSelected) {
