...
public class InstanceChooserTabs extends TabActivity {
	...
	protected void onCreate(final Bundle savedInstanceState) {
		...
		final Intent saved = new Intent(this, InstanceChooserList.class);
		tabHost.addTab(tabHost.newTabSpec(SAVED_TAB)
				.setIndicator(getString(R.string.saved_data, mSavedCount))
				.setContent(saved));

		// create intent for completed tab
		final Intent completed = new Intent(this, InstanceChooserList.class);
		tabHost.addTab(tabHost
				.newTabSpec(COMPLETED_TAB)
				.setIndicator(
						getString(R.string.completed_data, mCompletedCount))
		...
		final RelativeLayout rls = (RelativeLayout) tw.getChildAt(0);
		final TextView tvs = (TextView) rls.getChildAt(1);
		tvs.setTextSize(FONT_SIZE);
		tvs.setPadding(0, 0, 0, 6);

		final RelativeLayout rlc = (RelativeLayout) tw.getChildAt(1);
		final TextView tvc = (TextView) rlc.getChildAt(1);
