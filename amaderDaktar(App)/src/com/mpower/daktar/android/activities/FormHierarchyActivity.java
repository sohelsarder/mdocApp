...
import com.mpower.daktar.android.adapters.HierarchyListAdapter;
import com.mpower.daktar.android.logic.FormController;
import com.mpower.daktar.android.logic.HierarchyElement;
...
public class FormHierarchyActivity extends ListActivity {
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		final Button jumpBeginningButton = (Button) findViewById(R.id.jumpBeginningButton);
		...
		final Button jumpEndButton = (Button) findViewById(R.id.jumpEndButton);
		...
		getListView().post(new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < getListAdapter().getCount(); i++) {
					final HierarchyElement he = (HierarchyElement) getListAdapter()
							.getItem(i);
	...
	private void goUpLevel() {
		FormIndex index = stepIndexOut(FormEntryActivity.mFormController
				.getFormIndex());
		final int currentEvent = FormEntryActivity.mFormController.getEvent();
		...
		if (index == null) {
			...
			if (currentEvent == FormEntryController.EVENT_REPEAT) {
				// We were at a repeat, so stepping back brought us to then
				// previous level
	...
	public void refreshView() {
		...
		// user hits 'back'.
		final FormIndex currentIndex = FormEntryActivity.mFormController
				.getFormIndex();
		...
		if (FormEntryActivity.mFormController.getEvent() == FormEntryController.EVENT_REPEAT) {
			enclosingGroupRef = FormEntryActivity.mFormController
					.getFormIndex().getReference().toString(false);
			FormEntryActivity.mFormController
		...
		event_search: while (event != FormEntryController.EVENT_END_OF_FORM) {
			...
			case FormEntryController.EVENT_QUESTION:
				if (!repeatedGroupRef.equalsIgnoreCase("")) {
					...
					// move to the next
					// index.
					event = FormEntryActivity.mFormController
							.stepToNextEvent(FormController.STEP_OVER_GROUP);
					continue;
				}

				final FormEntryPrompt fp = FormEntryActivity.mFormController
						.getQuestionPrompt();
				formList.add(new HierarchyElement(fp.getLongText(), fp
						.getAnswerText(), null, Color.WHITE, QUESTION, fp
						.getIndex()));
				break;
			...
			case FormEntryController.EVENT_PROMPT_NEW_REPEAT:
				if (enclosingGroupRef
						.compareTo(FormEntryActivity.mFormController
			...
			case FormEntryController.EVENT_REPEAT:
				final FormEntryCaption fc = FormEntryActivity.mFormController
				.getCaptionPrompt();
				if (enclosingGroupRef
						.compareTo(FormEntryActivity.mFormController
								.getFormIndex().getReference().toString(false)) == 0) {
				...
				if (repeatedGroupRef.equalsIgnoreCase("")
					...
					// of its children
					final HierarchyElement group = new HierarchyElement(
							fc.getLongText(), null, getResources().getDrawable(
									R.drawable.expander_ic_minimized),
									Color.WHITE, COLLAPSED, fc.getIndex());
					repeatedGroupRef = FormEntryActivity.mFormController
							.getFormIndex().getReference().toString(false);
					formList.add(group);
				}
		...
		final HierarchyListAdapter itla = new HierarchyListAdapter(this);
	 ...
	 * @return index
	 */
	public FormIndex stepIndexOut(final FormIndex index) {
		if (index.isTerminal())
			return null;
		else
	...
	protected void onListItemClick(final ListView l, final View v,
		...
		final HierarchyElement h = (HierarchyElement) l
		...
		case EXPANDED:
			h.setType(COLLAPSED);
			final ArrayList<HierarchyElement> children = h.getChildren();
		...
		case COLLAPSED:
			h.setType(EXPANDED);
			final ArrayList<HierarchyElement> children1 = h.getChildren();
