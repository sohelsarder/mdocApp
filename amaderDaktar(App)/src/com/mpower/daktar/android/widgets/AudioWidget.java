...
public class AudioWidget extends QuestionWidget implements IBinaryWidget {
	...
	public AudioWidget(final Context context, final FormEntryPrompt prompt) {
			...
			public void onClick(final View v) {
				...
				try {
					...
					Toast.makeText(
							getContext(),
							getContext().getString(R.string.activity_not_found,
									"choose audio"), Toast.LENGTH_SHORT);
				}
		...
		mPlayButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(final View v) {
				...
				final File f = new File(mInstanceFolder + "/" + mBinaryName);
	...
	private String getPathFromUri(final Uri uri) {
		if (uri.toString().startsWith("file"))
			...
			final String[] audioProjection = { MediaColumns.DATA };
			final Cursor c = ((Activity) getContext()).managedQuery(uri,
					audioProjection, null, null, null);
			((Activity) getContext()).startManagingCursor(c);
			final int column_index = c.getColumnIndexOrThrow(MediaColumns.DATA);
	...
	@Override
	public void setBinaryData(final Object binaryuri) {
		...
		final String binaryPath = getPathFromUri((Uri) binaryuri);
		final String extension = binaryPath.substring(binaryPath
				.lastIndexOf("."));
		final String destAudioPath = mInstanceFolder + "/"
				+ System.currentTimeMillis() + extension;

		final File source = new File(binaryPath);
		final File newAudio = new File(destAudioPath);
		...
		if (newAudio.exists()) {
			...
			final Uri AudioURI = getContext().getContentResolver().insert(
