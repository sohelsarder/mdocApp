...
/**
 * ListMultiWidget handles multiple selection fields using check boxes. The
 * check boxes are aligned horizontally. They are typically meant to be used in
 * a field list, where multiple questions with the same multiple choice answers
 * can sit on top of each other and make a grid of buttons that is easy to
 * navigate quickly. Optionally, you can turn off the labels. This would be done
 * if a label widget was at the top of your field list to provide the labels. If
 * audio or video are specified in the select answers they are ignored. This
 * class is almost identical to ListWidget, except it uses checkboxes. It also
 * did not require a custom clickListener class.
 *
...
public class ListMultiWidget extends QuestionWidget {
	...
	public ListMultiWidget(final Context context, final FormEntryPrompt prompt,
