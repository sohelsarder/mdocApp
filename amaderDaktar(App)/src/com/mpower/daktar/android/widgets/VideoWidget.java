...
public class VideoWidget extends QuestionWidget implements IBinaryWidget {
	...
	public VideoWidget(final Context context, final FormEntryPrompt prompt) {
			...
			public void onClick(final View v) {
				...
				try {
					...
					Toast.makeText(
							getContext(),
							getContext().getString(R.string.activity_not_found,
									"video video"), Toast.LENGTH_SHORT);
				}
			}
		});
	...
	private String getPathFromUri(final Uri uri) {
		if (uri.toString().startsWith("file"))
			...
			final String[] videoProjection = { MediaColumns.DATA };
	...
	public void setBinaryData(final Object binaryuri) {
		...
		final String extension = binaryPath.substring(binaryPath
				.lastIndexOf("."));
		final String destVideoPath = mInstanceFolder + "/"
				+ System.currentTimeMillis() + extension;

		final File source = new File(binaryPath);
		final File newVideo = new File(destVideoPath);
		...
		if (newVideo.exists()) {
			...
			final Uri VideoURI = getContext().getContentResolver().insert(
