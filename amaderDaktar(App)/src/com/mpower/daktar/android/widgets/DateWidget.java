...
public class DateWidget extends QuestionWidget {
	...
	public DateWidget(final Context context, final FormEntryPrompt prompt) {
		...
		mDateListener = new DatePicker.OnDateChangedListener() {
			...
			public void onDateChanged(final DatePicker view, final int year,
				...
				if (mPrompt.isReadOnly()) {
					...
					c.set(year, month, 1);
					final int max = c.getActualMaximum(Calendar.DAY_OF_MONTH);
					// in older versions of android (1.6ish) the datepicker lets
					// you pick bad dats
					// in newer versions, calling update.Date() calls
					// onDatechangedListener(), causing an
					// endlessl loop.
					if (day > max) {
						if (!(mDatePicker.getDayOfMonth() == day
								&& mDatePicker.getMonth() == month && mDatePicker
								.getYear() == year)) {
	...
	private void setAnswer() {
		...
		if (mPrompt.getAnswerValue() != null) {
			final DateTime ldt = new DateTime(
					...
					.getTime());
			mDatePicker.init(ldt.getYear(), ldt.getMonthOfYear() - 1,
					ldt.getDayOfMonth(), mDateListener);
		} else {
	...
	public IAnswerData getAnswer() {
		final DateTime ldt = new DateTime(mDatePicker.getYear(),
				mDatePicker.getMonth() + 1, mDatePicker.getDayOfMonth(), 0, 0);
