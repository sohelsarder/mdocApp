...
public class ImageWidget extends QuestionWidget implements IBinaryWidget {
	...
	public ImageWidget(final Context context, final FormEntryPrompt prompt) {
		...
		instructions.setTextSize(TypedValue.COMPLEX_UNIT_DIP, mAnswerFontsize);
		instructions.setTextColor(Color.rgb(0, 122, 0));
		...
		// Set up an instruction which tells RMPs to swipe if satisfied with
		// picture
		instructions.setText(R.string.image_instructions);
		...
		if (mBinaryName != null) {
			...
			if (f.exists()) {
				final Bitmap bmp = FileUtils.getBitmapScaledToDisplay(f,
	...
	private String getPathFromUri(final Uri uri) {
		if (uri.toString().startsWith("file"))
			...
			final Cursor c = getContext().getContentResolver().query(uri, null,
					null, null, null);
			c.moveToFirst();

			// get data path
			final String colString = c.getString(c.getColumnIndex("_data"));
			c.close();
			return colString;
		}
	}
	...
	public void setBinaryData(final Object binaryuri) {
		...
		final File f = new File(binarypath);
