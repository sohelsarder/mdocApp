 ...
 * below. The full list of possible answers is not displayed to the user. The
 * goal is to be more compact; this question type is best suited for select one
 * questions with a large number of possible answers. If images, audio, or video
 * are specified in the select answers they are ignored.
 *
...
public class AutoCompleteWidget extends QuestionWidget {
	...
	public AutoCompleteWidget(final Context context,
			final FormEntryPrompt prompt, final String filterType) {
		super(context, prompt);
		mItems = prompt.getSelectChoices();
		mPrompt = prompt;

		choices = new AutoCompleteAdapter(getContext(),
		...
		for (final SelectChoice sc : mItems) {
	...
	public IAnswerData getAnswer() {
		final String response = autocomplete.getText().toString();
		for (final SelectChoice sc : mItems) {
			if (response.equals(mPrompt.getSelectChoiceText(sc)))
				return new SelectOneData(new Selection(sc));
		}

		// If the user has typed text into the autocomplete box that doesn't
		// match any answer, warn
		// them that their
		...
		if (!response.equals("")) {
			Toast.makeText(
					...
					"Warning: \""
							+ response
							+ "\" does not match any answers. No answer recorded.",
	...
	Filterable {
		...
		private class ItemsFilter extends Filter {
			...
			public ItemsFilter(final ArrayList<String> list) {
				if (list == null) {
				...
				if (prefix == null || prefix.length() == 0) {
					...
					for (int i = 0; i < count; i++) {
						...
						if (filterType.equals(match_substring)
							...
							final char[] toMatch = prefixString.toCharArray();

							boolean matches = true;
							for (int j = 0; j < toMatch.length; j++) {
								final int index = item_compare
										.indexOf(toMatch[j]);
								if (index > -1) {
									item_compare = item_compare.substring(0,
											index)
