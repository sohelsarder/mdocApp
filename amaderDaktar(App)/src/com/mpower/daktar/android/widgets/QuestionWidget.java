...
public abstract class QuestionWidget extends LinearLayout {
	...
	public QuestionWidget(final Context context, final FormEntryPrompt p) {
		...
		final String question_font = settings.getString(
				PreferencesActivity.KEY_FONT_SIZE, MIntel.DEFAULT_FONTSIZE);
	...
	protected void addQuestionText(final FormEntryPrompt p) {
		...
		mQuestionText.setTextSize(TypedValue.COMPLEX_UNIT_DIP,
				mQuestionFontsize);
	 ...
	 */
	private void addHelpText(final FormEntryPrompt p) {

		final String s = p.getHelpText();

		if (s != null && !s.equals("")) {
			mHelpText = new TextView(getContext());
			mHelpText.setTextSize(TypedValue.COMPLEX_UNIT_DIP,
					mQuestionFontsize - 3);
