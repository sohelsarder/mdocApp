...
package com.mpower.daktar.android.provider;
...
import com.mpower.daktar.android.database.MIntelSQLiteOpenHelper;
...
public class FormsProvider extends ContentProvider {
	...
	private static final String DATABASE_PATH = MIntel.METADATA_PATH;
	 ...
	 */
	private static class DatabaseHelper extends MIntelSQLiteOpenHelper {

		DatabaseHelper(final String databasePath, final String databaseName) {
			super(databasePath, databaseName, null, DATABASE_VERSION);
		}
		...
		public void onCreate(final SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + FORMS_TABLE_NAME + " ("
					...
					+ FormsColumns.MODEL_VERSION
					+ " integer, "
					+ FormsColumns.UI_VERSION
					+ " integer, "
	...
	public boolean onCreate() {
		mDbHelper = new DatabaseHelper(DATABASE_PATH, DATABASE_NAME);
		return true;
	}
	...
	public Uri insert(final Uri uri, final ContentValues initialValues) {
		...
		if (values.containsKey(FormsColumns.DISPLAY_SUBTEXT) == false) {
			...
			final String ts = new SimpleDateFormat(
					"EEE, MMM dd, yyyy 'at' HH:mm").format(today);
		...
		if (values.containsKey(FormsColumns.FORM_FILE_PATH) == true) {
			final String filePath = values
					.getAsString(FormsColumns.FORM_FILE_PATH);
			final File form = new File(filePath);
		...
		final long rowId = db.insert(FORMS_TABLE_NAME, null, values);
		if (rowId > 0) {
			final Uri formUri = ContentUris.withAppendedId(
					FormsColumns.CONTENT_URI, rowId);
	...
	private void deleteFileOrDir(final String fileName) {
	...
	public int delete(final Uri uri, final String where,
		...
		switch (sUriMatcher.match(uri)) {
		case FORMS:
			final Cursor del = this.query(uri, null, where, whereArgs, null);
		...
		case FORM_ID:
			final String formId = uri.getPathSegments().get(1);

			final Cursor c = this.query(uri, null, where, whereArgs, null);
			...
			count = db.delete(FORMS_TABLE_NAME,
					BaseColumns._ID
					+ "="
					+ formId
	...
	public int update(final Uri uri, final ContentValues values,
		...
		case FORMS:
			...
			if (c.getCount() > 0) {
				...
				while (c.moveToNext()) {
					...
					if (values.containsKey(FormsColumns.FORM_FILE_PATH)) {
						final String newFile = values
								.getAsString(FormsColumns.FORM_FILE_PATH);
						final String delFile = c.getString(c
								.getColumnIndex(FormsColumns.FORM_FILE_PATH));
		...
		case FORM_ID:
			...
			final Cursor update = this.query(uri, null, where, whereArgs, null);
			...
			if (update.getCount() > 0) {
				...
				// the order here is important (jrcache needs to be before form
				// file)
				...
				if (values.containsKey(FormsColumns.FORM_FILE_PATH)) {
					final String formFile = values
							.getAsString(FormsColumns.FORM_FILE_PATH);
					final String oldFile = update.getString(update
							.getColumnIndex(FormsColumns.FORM_FILE_PATH));

					if (formFile != null && formFile.equalsIgnoreCase(oldFile)) {
						// Files are the same, so we may have just copied over
						// something we had
						// already
					} else {
					...
					deleteFileOrDir(update.getString(update
							.getColumnIndex(FormsColumns.JRCACHE_FILE_PATH)));
					final String newMd5 = FileUtils.getMd5Hash(new File(
							formFile));
					values.put(FormsColumns.MD5_HASH, newMd5);
					values.put(FormsColumns.JRCACHE_FILE_PATH,
							MIntel.CACHE_PATH + newMd5 + ".formdef");
				}
				...
				count = db.update(FORMS_TABLE_NAME, values, BaseColumns._ID
						+ "="
						+ formId
						+ (!TextUtils.isEmpty(where) ? " AND (" + where + ')'
								: ""), whereArgs);
	...
	static {
		...
		sFormsProjectionMap.put(FormsColumns.MODEL_VERSION,
				FormsColumns.MODEL_VERSION);
		sFormsProjectionMap.put(FormsColumns.UI_VERSION,
				FormsColumns.UI_VERSION);
