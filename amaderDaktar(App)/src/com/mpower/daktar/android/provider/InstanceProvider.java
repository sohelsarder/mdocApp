...
public class InstanceProvider extends ContentProvider {
	...
	private static final String DATABASE_LOCATION = MIntel.METADATA_PATH;
	...
	private static class DatabaseHelper extends MIntelSQLiteOpenHelper {
		...
		DatabaseHelper(final String databaseLocation, final String databaseName) {
			super(databaseLocation, databaseName, null, DATABASE_VERSION);
		}
		...
		public void onCreate(final SQLiteDatabase db) {
			db.execSQL("CREATE TABLE " + INSTANCES_TABLE_NAME + " ("
					...
					+ " date not null, " + InstanceColumns.DISPLAY_SUBTEXT
					+ " text not null );");
		}
	...
	public boolean onCreate() {
		mDbHelper = new DatabaseHelper(DATABASE_LOCATION, DATABASE_NAME);
		return true;
	}
	...
	public Uri insert(final Uri uri, final ContentValues initialValues) {
		...
		if (values.containsKey(InstanceColumns.DISPLAY_SUBTEXT) == false) {
			...
			final String text = getDisplaySubtext(
					InstanceProviderAPI.STATUS_INCOMPLETE, today);
		...
		final long rowId = db.insert(INSTANCES_TABLE_NAME, null, values);
		if (rowId > 0) {
			final Uri instanceUri = ContentUris.withAppendedId(
					InstanceColumns.CONTENT_URI, rowId);
	...
	private String getDisplaySubtext(final String state, final Date date) {
		final String ts = new SimpleDateFormat("EEE, MMM dd, yyyy 'at' HH:mm")
		.format(date);
		if (state == null)
			return "Added on " + ts;
	...
	public int delete(final Uri uri, final String where,
		...
		case INSTANCES:
			...
			while (del.moveToNext()) {
				final String instanceFile = del.getString(del
						.getColumnIndex(InstanceColumns.INSTANCE_FILE_PATH));
				final String instanceDir = new File(instanceFile).getParent();
	...
	public int update(final Uri uri, final ContentValues values,
		...
		case INSTANCES:
			if (values.containsKey(InstanceColumns.STATUS)) {
				...
				if (values.containsKey(InstanceColumns.DISPLAY_SUBTEXT) == false) {
					final Date today = new Date();
					final String text = getDisplaySubtext(status, today);
					values.put(InstanceColumns.DISPLAY_SUBTEXT, text);
				}
			}
	...
	static {
		...
		sInstancesProjectionMap.put(InstanceColumns.DISPLAY_NAME,
				InstanceColumns.DISPLAY_NAME);
		sInstancesProjectionMap.put(InstanceColumns.SUBMISSION_URI,
		...
		sInstancesProjectionMap.put(InstanceColumns.JR_FORM_ID,
				InstanceColumns.JR_FORM_ID);
		sInstancesProjectionMap.put(InstanceColumns.STATUS,
