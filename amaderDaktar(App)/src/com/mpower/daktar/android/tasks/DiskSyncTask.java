...
public class DiskSyncTask extends AsyncTask<Void, String, String> {
	...
	protected String doInBackground(final Void... params) {
		// get all forms
		final Cursor mCursor = MIntel.getInstance().getContentResolver()
		...
		final File formDir = new File(MIntel.FORMS_PATH);
		if (formDir.exists() && formDir.isDirectory()) {
			// This is all the files in the /mintel/forms directory
			final ArrayList<File> xFormsToAdd = new ArrayList<File>(
					Arrays.asList(formDir.listFiles()));

			while (mCursor.moveToNext()) {
				// For each element in the provider, see if the file already
				// exists
				final String sqlFilename = mCursor.getString(mCursor
						.getColumnIndex(FormsColumns.FORM_FILE_PATH));
				final String md5 = mCursor.getString(mCursor
						.getColumnIndex(FormsColumns.MD5_HASH));
				final File sqlFile = new File(sqlFilename);
				if (sqlFile.exists()) {
					// remove it from the list of forms (we only want forms we
					// haven't added at the
					// end)
					...
					if (!FileUtils.getMd5Hash(sqlFile).contentEquals(md5)) {
						...
						final Uri updateUri = Uri.withAppendedPath(
						...
						final String title = fields.get(FileUtils.TITLE);
						final String ui = fields.get(FileUtils.UI);
						final String model = fields.get(FileUtils.MODEL);
						final String formid = fields.get(FileUtils.FORMID);
						final String submission = fields
								.get(FileUtils.SUBMISSIONURI);

						// update date
						final Long now = Long.valueOf(System
								.currentTimeMillis());
						...
						if (title != null) {
							...
							return MIntel.getInstance().getString(
									R.string.xform_parse_error,
						...
						if (submission != null) {
							updateValues.put(FormsColumns.SUBMISSION_URI,
									submission);
						}
						// Note, the path doesn't change here, but it needs to
						// be included so the
						// update will automatically update the .md5 and the
						// cache path.
						updateValues.put(FormsColumns.FORM_FILE_PATH,
								sqlFile.getAbsolutePath());
						final int count = MIntel.getInstance()
								.getContentResolver()
			...
			// Whatever is left in our arraylist isn't in the database, so add
			// it
			for (int i = 0; i < xFormsToAdd.size(); i++) {
				final ContentValues values = new ContentValues();
				final File addMe = xFormsToAdd.get(i);
				...
				if (!addMe.getName().startsWith(".")
					...
					if (title != null) {
						...
						return MIntel.getInstance().getString(
								R.string.xform_parse_error, addMe.getName(),
								"title");
					if (formid != null) {
		...
		return MIntel.getInstance().getString(R.string.finished_disk_scan);

	}

	public void setDiskSyncListener(final DiskSyncListener l) {
		mListener = l;
	}
