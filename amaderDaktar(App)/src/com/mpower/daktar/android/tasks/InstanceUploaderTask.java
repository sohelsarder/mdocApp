...
AsyncTask<Long, Integer, HashMap<String, String>> {
	...
	protected HashMap<String, String> doInBackground(final Long... values) {
		...
		if (c.getCount() > 0) {
			...
			next_submission: while (c.moveToNext()) {
				...
				final Uri toUpdate = Uri.withAppendedPath(
						InstanceColumns.CONTENT_URI, id);

				String urlString = c.getString(c
				...
				URI u = null;
				try {
					final URL url = new URL(URLDecoder.decode(urlString,
							"utf-8"));
					u = url.toURI();
				} catch (final MalformedURLException e) {
					e.printStackTrace();
					mResults.put(id, fail + "invalid url: " + urlString
					...
					e.printStackTrace();
					mResults.put(id, fail + "invalid uri: " + urlString
							+ " :: details: " + e.getMessage());
					cv.put(InstanceColumns.STATUS,
							InstanceProviderAPI.STATUS_SUBMISSION_FAILED);
					MIntel.getInstance().getContentResolver()
				...
				if (uriRemap.containsKey(u)) {
					...
					final HttpHead httpHead = WebUtils
							.createOpenRosaHttpHead(u);
					...
					try {
						...
						if (statusCode == 401) {
							...
							final Header[] locations = response
									.getHeaders("Location");
							...
							Log.w(t, "Status code on Head request: "
									+ statusCode);
				...
				final File[] allFiles = instanceFile.getParentFile()
						.listFiles();
				...
				while (j < files.size() || first) {
					...
					final HttpPost httppost = WebUtils.createOpenRosaHttpPost(
							u, mAuth);
					...
					for (; j < files.size(); j++) {
						...
						if (extension.equals("xml")) {
							...
							entity.addPart(f.getName(), fb);
							byteCount += f.length();
							Log.i(t, "added recognized filetype ("
									+ contentType + ") " + f.getName());
						} else {
						...
						if (j + 1 < files.size()) {
							if (byteCount + files.get(j + 1).length() > 10000000L) {
								...
								try {
									final StringBody sb = new StringBody("yes",
					...
					try {
						...
						try {
							postResponse = EntityUtils.toString(httpEntity,
									HTTP.UTF_8).trim();
						} catch (final IOException e) {
							e.printStackTrace();
						}
	...
	public void setUploaderListener(final InstanceUploaderListener sl) {
		synchronized (this) {
			mStateListener = sl;
		}
	}
