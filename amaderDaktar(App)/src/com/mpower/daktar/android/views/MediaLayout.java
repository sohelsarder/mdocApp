...
public class MediaLayout extends RelativeLayout {
	...
	public void setAVT(final TextView text, final String audioURI,
			final String imageURI, final String videoURI,
		...
		final RelativeLayout.LayoutParams audioParams = new RelativeLayout.LayoutParams(
				android.view.ViewGroup.LayoutParams.WRAP_CONTENT,
				android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
		...
		if (audioURI != null) {
			...
			mAudioButton.setId(3245345); // random ID to be used by the relative
			// layout.
		} else {
		...
		if (videoURI != null) {
			...
			mVideoButton.setOnClickListener(new OnClickListener() {
				...
				public void onClick(final View v) {
					...
					try {
						videoFilename = ReferenceManager._()
								.DeriveReference(videoURI).getLocalURI();
					...
					if (!videoFile.exists()) {
						// We should have a video clip, but the file doesn't
						// exist.
						final String errorMsg = getContext().getString(
								R.string.file_missing, videoFilename);
						Log.e(t, errorMsg);
		...
		final boolean textVisible = text.getVisibility() != GONE;
		if (textVisible) {
		...
		if (imageURI != null) {
			try {
				...
				if (imageFile.exists()) {
					...
					if (b != null) {
						...
						if (mAudioButton != null) {
							if (textVisible) {
								...
								imageParams.addRule(RelativeLayout.LEFT_OF,
										mAudioButton.getId());
							}
						}
						...
						if (bigImageURI != null) {
							...
							.setOnClickListener(new OnClickListener() {
								String bigImageFilename = ReferenceManager
										._()
						...
						// An error hasn't been logged and loading the image
						// failed, so it's likely
						// a bad file.
					...
					// An error hasn't been logged. We should have an image, but
					// the file doesn't
					// exist.
					errorMsg = getContext().getString(R.string.file_missing,
							imageFile);
				}
	...
	/**
	 * This adds a divider at the bottom of this layout. Used to separate fields
	 * in lists.
	 *
	 * @param v
	 */
	public void addDivider(final ImageView v) {
		final RelativeLayout.LayoutParams dividerParams = new RelativeLayout.LayoutParams(
				android.view.ViewGroup.LayoutParams.FILL_PARENT,
				android.view.ViewGroup.LayoutParams.WRAP_CONTENT);
		if (mImageView != null) {
